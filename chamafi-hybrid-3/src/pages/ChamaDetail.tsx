...
export function ChamaDetail() {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6 animate-fade-in">
      ...
      <Link
        to="/app/discover"
        className="inline-flex items-center gap-2 text-sand/50 hover:text-sand transition-colors text-sm"
      >
        <ArrowLeft className="w-4 h-4" />
        Back to Discover
      </Link>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        />
        <StatCard
          icon={<Target className="w-5 h-5 text-lime" />}
          label="Target"
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <div className="space-y-6">
          ...
          {!isMember && !graduated && !expired && (
            <ActionCard title="Join Chama" icon={<Users className="w-5 h-5 text-lime" />}>
          ...
          {/* Contribute Section */}
          {isMember && !graduated && !expired && (
            <ActionCard title="Contribute" icon={<Coins className="w-5 h-5 text-lime" />}>
              <div className="space-y-4">
                ...
                {needsApproval && (
                  <p className="text-xs text-sun-gold/80">
          ...
          {isCreator && targetReached && !graduated && !expired && (
            <ActionCard title="Graduate Chama" icon={<GraduationCap className="w-5 h-5 text-sun-gold" />}>
              <p className="text-sand/50 text-sm mb-4">
          ...
          {isMember && expired && !graduated && hasContributed && (
            <ActionCard title="Claim Refund" icon={<AlertTriangle className="w-5 h-5 text-sun-gold" />}>
              <p className="text-sand/50 text-sm mb-4">
            ...
            <div className="space-y-2 max-h-64 overflow-y-auto pr-1">
              {members && members.length > 0 ? (
                members.map((member) => (
                  <div
                    key={member}
                    className="flex items-center gap-2 py-2.5 px-3 bg-white/[0.03] rounded-xl border border-sand/5"
                  >
                    <div className="w-7 h-7 rounded-lg bg-clay/30 flex items-center justify-center">
                      <User className="w-3.5 h-3.5 text-sand/50" />
                    </div>
                    <span className="font-mono text-sm text-sand/70 flex-1">
...
function StatCard({ icon, label, value }: {
  ...
  return (
    <div className="glass-card p-4 border-sand/5">
...
function ActionCard({ title, icon, children }: {
  ...
  return (
    <div className="glass-card p-6 border-sand/5">
      <h3 className="font-semibold text-cream mb-4 flex items-center gap-2">
        {icon}
        {title}
      </h3>
